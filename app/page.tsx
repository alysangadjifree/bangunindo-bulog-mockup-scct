...
type DashboardManagementMode = "dashboards" | "widgets" | "menus" | "savedViews" | "executiveLayout";
...
const sidebarSections: SidebarSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Forecast Report", icon: ChartNoAxesCombined, children: ["Demand & Supply Forecast", "Inventory Forecast", "Forecast Accuracy"] },
...
const enabledNavigation = new Set([
  ...
  "Dashboard Management", "Dashboard", "Widget", "Menu", "Saved View", "Executive Layout",
...
const forecastRegions=[
  ...
  {region:"Sulawesi",demand:426,supply:458,gap:32,coverage:55,risk:"Aman",confidence:90},
  {region:"Bali & Nusa Tenggara",demand:282,supply:241,gap:-41,coverage:38,risk:"Monitor",confidence:88},
  {region:"Kalimantan",demand:243,supply:216,gap:-27,coverage:35,risk:"Monitor",confidence:87},
  {region:"Maluku & Papua",demand:254,supply:178,gap:-76,coverage:24,risk:"Kritis",confidence:84},
];

function ForecastReportPage({mode,onSwitch,onNotify}:{mode:ForecastReportMode;onSwitch:(mode:ForecastReportMode)=>void;onNotify:(message:string)=>void}){
  const [horizon,setHorizon]=useState("90 hari");
  const [commodity,setCommodity]=useState("Beras CBP");
  const [selectedRegion,setSelectedRegion]=useState("Nasional");
  const [running,setRunning]=useState(false);
  const [lastRun,setLastRun]=useState("20 Agustus 2026 · 09:40 WIB");
  const tabs:[ForecastReportMode,string][]=[["demandSupply","Demand & Supply Forecast"],["inventory","Inventory Forecast"],["accuracy","Forecast Accuracy"]];
  const titles:Record<ForecastReportMode,[string,string]>={demandSupply:["Demand & Supply Forecast","Proyeksi kebutuhan, pengadaan, dan penyaluran untuk mendeteksi gap per wilayah sebelum mengganggu pelayanan."],inventory:["Inventory Forecast","Proyeksi posisi stok, safety stock, kapasitas, dan aging berdasarkan arus masuk-keluar serta penugasan pemerintah."],accuracy:["Forecast Accuracy","Evaluasi WAPE, bias, kestabilan model, dan kualitas data agar forecast layak digunakan dalam keputusan BULOG."]};
  const runForecast=()=>{setRunning(true);window.setTimeout(()=>{setRunning(false);setLastRun("Baru saja");onNotify("Forecast diperbarui dengan baseline operasional terbaru")},850)};
  return <main className="forecast-report-page">
    <header className="forecast-header"><div><span>REPORT &amp; GOVERNANCE / FORECAST REPORT</span><h1>{titles[mode][0]}</h1><p>{titles[mode][1]}</p></div><div className="forecast-header-actions"><span><i/><small>Forecast terakhir</small><strong>{lastRun}</strong></span><button onClick={()=>onNotify("Forecast report diekspor ke PDF dan XLSX")}><Download size={15}/>Ekspor</button><button className="primary" onClick={runForecast} disabled={running}><RotateCw className={running?"spin":""} size={15}/>{running?"Menghitung…":"Jalankan Forecast"}</button></div></header>
    <nav className="forecast-tabs">{tabs.map(([id,label])=><button key={id} className={mode===id?"active":""} onClick={()=>onSwitch(id)}>{label}</button>)}</nav>
    <section className="forecast-toolbar"><label><span>Komoditas</span><select value={commodity} onChange={e=>setCommodity(e.target.value)}><option>Beras CBP</option><option>Beras Medium</option><option>Beras Premium</option><option>Jagung</option></select></label><label><span>Cakupan</span><select value={selectedRegion} onChange={e=>setSelectedRegion(e.target.value)}><option>Nasional</option>{forecastRegions.map(x=><option key={x.region}>{x.region}</option>)}</select></label><label><span>Horizon</span><select value={horizon} onChange={e=>setHorizon(e.target.value)}><option>30 hari</option><option>60 hari</option><option>90 hari</option><option>180 hari</option></select></label><div><span>Baseline aktif</span><strong>WMS + ERP + BPS + program 2026</strong><small>Versi FCT-2026.08.20</small></div></section>
    {mode==="demandSupply"&&<>
      <section className="forecast-kpis"><article><span>Forecast demand</span><strong>3,08 jt ton</strong><small>{horizon} · seluruh program</small><em className="watch">+8,4% vs baseline</em></article><article><span>Forecast supply</span><strong>2,74 jt ton</strong><small>pengadaan + mutasi masuk</small><em className="risk">Gap 340 rb ton</em></article><article><span>Stok tersedia</span><strong>5,25 jt ton</strong><small>cut-off operasional SCCT</small><em className="good">Buffer nasional kuat</em></article><article><span>Wilayah berisiko</span><strong>4 region</strong><small>coverage &lt; 45 hari</small><em className="risk">1 kritis</em></article><article><span>Confidence</span><strong>90,1%</strong><small>weighted national</small><em className="good">Layak operasional</em></article></section>
      <div className="forecast-grid"><section className="forecast-card demand-supply-chart"><header><div><span>PROYEKSI BULANAN</span><h2>Demand, supply, dan gap nasional</h2></div><small>ribu ton · confidence interval 80%</small></header><div className="forecast-bars">{[["Agustus",820,790],["September",925,846],["Oktober",1010,902],["November",1125,980],["Desember",1280,1056]].map(([month,demand,supply])=><article key={month as string}><span>{month}</span><div><i><b style={{width:`${Number(demand)/13}%`}}/><em style={{width:`${Number(supply)/13}%`}}/></i><small><b>{demand}</b> demand · <b>{supply}</b> supply</small></div><strong className={Number(supply)<Number(demand)?"negative":"positive"}>{Number(supply)-Number(demand)} rb</strong></article>)}</div><footer><span><i/>Demand forecast</span><span><i/>Supply forecast</span><p>Gap melebar menjelang akhir tahun; intervensi perlu dikunci sebelum lead time distribusi wilayah timur terlampaui.</p></footer></section><aside className="forecast-card forecast-actions"><header><div><span>REKOMENDASI TINDAKAN</span><h2>Prioritas 14 hari</h2></div><Sparkles size={19}/></header>{[["01","Papua & Maluku","Reservasi 76 rb ton dari Jawa/Sulawesi","Critical"],["02","Bali & Nusa Tenggara","Kunci slot kapal dan buffer 41 rb ton","High"],["03","Sumatera","Aktifkan tambahan titik serap 43 rb ton","High"],["04","Nasional","Review cadence SPHP terhadap harga wilayah","Watch"]].map(([no,area,action,status])=><article key={no}><b>{no}</b><span><strong>{area}</strong><small>{action}</small></span><em className={status.toLowerCase()}>{status}</em><button onClick={()=>onNotify(`${action} dikirim ke Scenario Workspace`)}><ChevronRight size={15}/></button></article>)}</aside></div>
      <section className="forecast-card forecast-region-table"><header><div><span>REGIONAL BALANCE</span><h2>Forecast keseimbangan per region</h2></div><button onClick={()=>onNotify("Case dibuat untuk wilayah berisiko")}><Plus size={14}/>Buat case dari risiko</button></header><div className="forecast-region-head"><span>Region</span><span>Demand</span><span>Supply</span><span>Balance</span><span>Coverage</span><span>Confidence</span><span>Status</span><span>Aksi</span></div>{forecastRegions.map(x=><article key={x.region}><strong>{x.region}</strong><span>{x.demand.toLocaleString("id-ID")} rb</span><span>{x.supply.toLocaleString("id-ID")} rb</span><b className={x.gap<0?"negative":"positive"}>{x.gap>0?"+":""}{x.gap} rb</b><span>{x.coverage} hari</span><span>{x.confidence}%</span><em className={x.risk.toLowerCase()}>{x.risk}</em><button onClick={()=>{setSelectedRegion(x.region);onNotify(`Drill-down ${x.region} dibuka`)}}>Analisis</button></article>)}</section>
    </>}
    {mode==="inventory"&&<>
      <section className="forecast-kpis"><article><span>Stok awal</span><strong>5,25 jt ton</strong><small>WMS · 20 Agustus</small><em className="good">92,23% kapasitas</em></article><article><span>Proyeksi stok akhir</span><strong>4,91 jt ton</strong><small>akhir {horizon}</small><em className="watch">−340 rb ton</em></article><article><span>Safety buffer</span><strong>1,18 jt ton</strong><small>di atas minimum nasional</small><em className="good">Terkendali</em></article><article><span>Stok &gt;4 bulan</span><strong>2,42 jt ton</strong><small>prioritas FEFO/QC</small><em className="risk">46% stok</em></article><article><span>Potensi overflow</span><strong>3 gudang</strong><small>Jawa · Bali/NT</small><em className="risk">Perlu outflow</em></article></section>
      <div className="forecast-grid inventory-forecast-grid"><section className="forecast-card inventory-waterfall"><header><div><span>INVENTORY WATERFALL</span><h2>Proyeksi stok nasional</h2></div><small>juta ton</small></header><div>{[["Stok awal",5.25,"base"],["Pengadaan",2.74,"in"],["SPHP",-.83,"out"],["Bantuan pangan",-1.16,"out"],["Komersial/CBP",-1.09,"out"],["Stok akhir",4.91,"total"]].map(([label,value,tone])=><article key={label as string}><strong>{Number(value)>0&&tone!=="base"&&tone!=="total"?"+":""}{Number(value).toLocaleString("id-ID")} jt</strong><i className={String(tone)} style={{height:`${Math.max(18,Math.abs(Number(value))*42)}px`}}/><span>{label}</span></article>)}</div><footer>Forecast memasukkan target SPHP 2026, penyaluran program, pengadaan berjalan, mutasi stok, dan guardrail safety stock.</footer></section><aside className="forecast-card aging-outlook"><header><div><span>AGING OUTLOOK</span><h2>Risiko mutu 90 hari</h2></div><ShieldCheck size={19}/></header>{[["0–2 bulan","1,39 jt",27,"Normal"],["2–4 bulan","1,44 jt",28,"Monitor"],[">4 bulan","2,42 jt",46,"Risiko"],["QC hold","45,2 rb",9,"Critical"]].map(([age,volume,pct,status])=><article key={age as string}><span><strong>{age}</strong><small>{volume} ton</small></span><i><b className={String(status).toLowerCase()} style={{width:`${pct}%`}}/></i><em>{pct}%</em><b>{status}</b></article>)}<button onClick={()=>onNotify("Rencana FEFO dibuka di Rice Outflow Optimizer")}>Optimalkan outflow FEFO <ArrowRight size={14}/></button></aside></div>
      <section className="forecast-card inventory-region-outlook"><header><div><span>REGIONAL INVENTORY OUTLOOK</span><h2>Coverage, kapasitas, dan kebutuhan intervensi</h2></div><button onClick={()=>onNotify("Skenario redistribusi dibuat")}><Route size={14}/>Buat skenario redistribusi</button></header>{forecastRegions.map(x=><article key={x.region}><span><strong>{x.region}</strong><small>Forecast akhir {horizon}</small></span><div><i><b style={{width:`${Math.min(100,x.coverage*1.35)}%`}}/></i><small>{x.coverage} hari coverage</small></div><em className={x.risk.toLowerCase()}>{x.risk}</em><button onClick={()=>onNotify(`Rencana replenishment ${x.region} diajukan`)}>Ajukan tindakan</button></article>)}</section>
    </>}
    {mode==="accuracy"&&<>
      <section className="forecast-kpis"><article><span>Demand WAPE</span><strong>7,8%</strong><small>target ≤10%</small><em className="good">Akurat</em></article><article><span>Supply WAPE</span><strong>10,6%</strong><small>target ≤12%</small><em className="good">Dalam batas</em></article><article><span>Inventory WAPE</span><strong>5,2%</strong><small>target ≤8%</small><em className="good">Akurat</em></article><article><span>Forecast bias</span><strong>−2,1%</strong><small>sedikit over-forecast</small><em className="watch">Monitor</em></article><article><span>Data readiness</span><strong>96,8%</strong><small>18 dari 19 sumber sehat</small><em className="good">Siap</em></article></section>
      <div className="forecast-grid accuracy-grid"><section className="forecast-card accuracy-trend"><header><div><span>BACKTEST 12 MINGGU</span><h2>Aktual vs forecast penyaluran</h2></div><small>WAPE mingguan</small></header><div>{[9.8,8.7,7.9,11.2,9.1,8.4,7.6,6.9,8.1,7.4,7.2,7.8].map((v,i)=><span key={i}><b style={{height:`${v*6}px`}}/><em style={{bottom:`${Math.min(70,42+i*1.4)}px`}}/><small>M{i+1}</small><strong>{v}%</strong></span>)}</div><footer><span><i/>WAPE aktual</span><span><i/>Target 10%</span><p>Performa membaik setelah kalender program dan sinyal harga wilayah ditambahkan ke model.</p></footer></section><aside className="forecast-card model-health"><header><div><span>MODEL HEALTH</span><h2>Guardrail operasional</h2></div><Gauge size={19}/></header>{[["Freshness sumber","96,8%","good"],["Completeness fitur","99,1%","good"],["Concept drift","3,4%","good"],["Wilayah low confidence","4","watch"],["Model overdue review","1","risk"]].map(([label,value,tone])=><article key={label}><span><strong>{label}</strong><small>{tone==="good"?"Dalam batas kendali":tone==="watch"?"Perlu validasi Kanwil":"Perlu review model"}</small></span><b className={tone}>{value}</b></article>)}<button onClick={()=>onNotify("Review model dikirim ke pemilik model")}>Ajukan model review</button></aside></div>
      <section className="forecast-card accuracy-table"><header><div><span>ACCURACY BY DOMAIN</span><h2>Kinerja model dan tindakan perbaikan</h2></div><button onClick={()=>onNotify("Laporan akurasi disimpan ke audit trail")}><Save size={14}/>Simpan evaluasi</button></header><div className="accuracy-head"><span>Model</span><span>WAPE</span><span>Bias</span><span>Confidence</span><span>Data issue</span><span>Status</span><span>Action</span></div>{[["Demand SPHP","6,9%","−1,4%","93%","0","Production","Pertahankan"],["Bantuan Pangan","8,2%","+2,8%","91%","1 kalender","Monitor","Update kalender"],["Pengadaan DN","10,6%","−4,1%","88%","2 freshness","Monitor","Retrain"],["Inventory Balance","5,2%","−0,8%","95%","0","Production","Pertahankan"],["Wilayah Timur","14,8%","−8,6%","79%","4 coverage","At Risk","Tambah sinyal"]].map(r=><article key={r[0]}>{r.slice(0,6).map((c,i)=>i===0?<strong key={c}>{c}</strong>:i===5?<em className={c.toLowerCase().replace(" ","-")} key={c}>{c}</em>:<span key={`${c}-${i}`}>{c}</span>)}<button onClick={()=>onNotify(`Action ${r[6]} dibuka`)}>{r[6]}</button></article>)}</section>
    </>}
    <footer className="forecast-source-note"><Database size={16}/><span><b>Basis informasi:</b> baseline operasional SCCT konsisten dengan dashboard demo; konteks nasional memakai publikasi resmi BULOG/BPS 2026. Angka proyeksi adalah hasil simulasi demonstratif dan wajib divalidasi pemilik data sebelum eksekusi.</span><button onClick={()=>onNotify("Data lineage forecast dibuka")}>Lihat data lineage</button></footer>
  </main>
}
...
function AlertConfigurationPage({mode,onSwitch,onNotify}:{mode:AlertConfigurationMode;onSwitch:(mode:AlertConfigurationMode)=>void;onNotify:(message:string)=>void}){
  ...
  const [severityEditor,setSeverityEditor]=useState<string|null>(null);
  ...
  <footer className="alert-config-disclaimer"><ShieldCheck size={16}/><span><b>Konfigurasi demonstrasi SCCT.</b> Nilai harus diselaraskan dengan kebijakan, struktur kewenangan, kanal resmi, kalender operasional, serta sumber WMS/ERP/TMS/Simotandi sebelum produksi.</span></footer>
  {severityEditor&&(()=>{const item=severityConfig.find(x=>x.level===severityEditor)!;return <div className="severity-editor-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setSeverityEditor(null)}}><section className="severity-editor" role="dialog" aria-modal="true" aria-labelledby="severity-editor-title" style={{"--severity-color":item.color} as CSSProperties}><header><div><span>EDIT SEVERITY GUARDRAIL</span><h2 id="severity-editor-title"><i/>{item.level}</h2><p>Atur klasifikasi dampak, target respons, penerima, dan proteksi perubahan untuk severity ini.</p></div><button onClick={()=>setSeverityEditor(null)} aria-label="Tutup popup"><X size={20}/></button></header><div className="severity-editor-body"><section><div className="severity-editor-section-title"><ShieldCheck size={17}/><span><strong>Kriteria klasifikasi</strong><small>Semua kondisi menggunakan logika OR kecuali ditentukan lain.</small></span></div><label className="full"><span>Definisi dan dampak bisnis</span><textarea defaultValue={item.criteria}/></label><div className="severity-editor-fields"><label><span>Eksposur finansial minimum</span><select defaultValue={item.level==="Critical"?"10000000000":item.level==="High"?"1000000000":"0"}><option value="0">Tidak digunakan</option><option value="1000000000">Rp1 miliar</option><option value="5000000000">Rp5 miliar</option><option value="10000000000">Rp10 miliar</option></select></label><label><span>Deviasi KPI minimum</span><div className="severity-editor-input"><input type="number" defaultValue={item.level==="Critical"?25:item.level==="High"?15:5}/><b>%</b></div></label><label><span>Gangguan layanan</span><select defaultValue={item.level==="Critical"?"national":item.level==="High"?"regional":"local"}><option value="national">Nasional / layanan publik</option><option value="regional">Regional / multi-unit</option><option value="local">Unit lokal</option><option value="none">Tanpa dampak langsung</option></select></label><label><span>Confidence minimum</span><div className="severity-editor-input"><input type="number" defaultValue="85"/><b>%</b></div></label></div><div className="severity-editor-checks"><label><input type="checkbox" defaultChecked={item.level==="Critical"||item.level==="High"}/><span><b>Override otomatis dari rule</b><small>Naikkan severity bila dampak aktual melewati threshold.</small></span></label><label><input type="checkbox" defaultChecked={item.level==="Critical"}/><span><b>Evidence wajib</b><small>Penurunan level harus menyertakan dokumen dan alasan.</small></span></label></div></section><section><div className="severity-editor-section-title"><Clock3 size={17}/><span><strong>Target penanganan & eskalasi</strong><small>Timer mengikuti kalender operasional policy terkait.</small></span></div><div className="severity-editor-fields three"><label><span>Acknowledge</span><input defaultValue={item.ack}/></label><label><span>Resolve</span><input defaultValue={item.resolve}/></label><label><span>Auto-escalate</span><input defaultValue={item.escalate}/></label></div><label className="full"><span>Penerima utama</span><input defaultValue={item.recipients}/></label><div className="severity-editor-fields"><label><span>Escalation policy</span><select defaultValue={item.level==="Critical"?"ESC-CR-01":"ESC-HI-02"}><option>ESC-CR-01</option><option>ESC-HI-02</option><option>ESC-OPS-04</option><option>Tidak ada</option></select></label><label><span>Notification route</span><select defaultValue={item.level==="Critical"?"NTF-001":"NTF-004"}><option>NTF-001</option><option>NTF-004</option><option>NTF-008</option></select></label></div></section><section className="severity-editor-preview"><header><span><i/>PREVIEW DAMPAK KONFIGURASI</span><em>Replay data 30 hari</em></header><div><article><small>Estimasi alert</small><strong>{item.level==="Critical"?"14–19":item.level==="High"?"28–36":"42–58"}</strong><span>per bulan</span></article><article><small>False positive</small><strong>{item.level==="Critical"?"2,1%":"3,8%"}</strong><span>target &lt;5%</span></article><article><small>Unit terdampak</small><strong>{item.level==="Critical"?"34":"18"}</strong><span>Kanwil</span></article></div><p><CheckCircle2 size={15}/><span>Tidak ditemukan konflik dengan SLA aktif, escalation resolver, atau threshold rule yang dipublikasikan.</span></p><button onClick={()=>onNotify(`Replay test ${item.level} dijalankan`)}><FlaskConical size={15}/>Jalankan replay test</button></section></div><footer><span><AlertTriangle size={15}/><small>Perubahan disimpan sebagai draf dan tidak langsung memengaruhi alert aktif.</small></span><button onClick={()=>setSeverityEditor(null)}>Batal</button><button onClick={()=>onNotify(`Draf guardrail ${item.level} disimpan`)}><Save size={15}/>Simpan Draf</button><button className="primary" onClick={()=>{setSeverityEditor(null);onNotify(`Guardrail ${item.level} diajukan untuk approval`)}}><Send size={15}/>Ajukan Approval</button></footer></section></div>})()}
  </main>;
}
...
type BlivFilterMap={key:string;nativeId:string;required:boolean;defaultValue:string};
type BlivDashboardRecord={id:string;name:string;domain:string;type:string;status:"Published"|"Draft"|"Review";version:string;owner:string;updated:string;height:number;widgets:number;placements:number;blivId:string;embedUrl:string;auth:string;refresh:number;description:string;roles:string[];organizations:string[];filters:BlivFilterMap[]};

const blivDashboards:BlivDashboardRecord[]=[
  {id:"DB-001",name:"National Persediaan",domain:"Persediaan",type:"Main Dashboard",status:"Published",version:"v12",owner:"Command Center",updated:"28 Agu 2026 · 21:14",height:6050,widgets:18,placements:3,blivId:"7ee04aa2-e9df-4add-8f18-f10ea4a1b0b3",embedUrl:"https://bliv-dashboard.bulog.internal/embed/persediaan",auth:"Signed guest token",refresh:300,description:"Peta stok nasional, kapasitas, aging, mutu, dan safety stock untuk kontrol harian.",roles:["Direksi","Command Center","Divisi Persediaan"],organizations:["Kantor Pusat","34 Kanwil"],filters:[{key:"filter_date_id",nativeId:"NATIVE_FILTER-CrzUMmZ0OrzSYU8qeKuI7",required:true,defaultValue:"YTD"},{key:"filter_region_id",nativeId:"NATIVE_FILTER-t3WVaNApxxS833ooNtGW7",required:false,defaultValue:"Nasional"},{key:"filter_kanwil_id",nativeId:"NATIVE_FILTER-PCzxXu--sYtt0yhGITv4I",required:false,defaultValue:"Semua"},{key:"filter_kancab_id",nativeId:"NATIVE_FILTER-9fHG2qpaQUErD8SQGTx6F",required:false,defaultValue:"Semua"}]},
  {id:"DB-002",name:"National Pengadaan",domain:"Pengadaan",type:"Main Dashboard",status:"Published",version:"v9",owner:"Divisi Pengadaan",updated:"28 Agu 2026 · 19:42",height:5480,widgets:15,placements:2,blivId:"1ad35d72-6066-4bea-a334-b3596fb87b11",embedUrl:"https://bliv-dashboard.bulog.internal/embed/pengadaan",auth:"Signed guest token",refresh:600,description:"Realisasi pengadaan, sumber pasokan, serapan wilayah, gap, dan proyeksi panen.",roles:["Direksi","Command Center","Divisi Pengadaan"],organizations:["Kantor Pusat","34 Kanwil"],filters:[{key:"filter_date_id",nativeId:"NATIVE_FILTER-PGD-DATE-01",required:true,defaultValue:"YTD"},{key:"filter_commodity_id",nativeId:"NATIVE_FILTER-PGD-KOM-02",required:false,defaultValue:"Beras"},{key:"filter_kanwil_id",nativeId:"NATIVE_FILTER-PGD-KW-03",required:false,defaultValue:"Semua"}]},
  {id:"DB-003",name:"Penjualan & Penyaluran",domain:"Penjualan",type:"Operational",status:"Published",version:"v8",owner:"Divisi Penjualan",updated:"28 Agu 2026 · 18:05",height:5120,widgets:14,placements:2,blivId:"da675640-9f80-4183-8a14-74abb75f77c7",embedUrl:"https://bliv-dashboard.bulog.internal/embed/penyaluran",auth:"Signed guest token",refresh:600,description:"SPHP, bantuan pangan, komersial, order fulfillment, dan kinerja penyaluran wilayah.",roles:["Direksi","Command Center","Divisi Penjualan"],organizations:["Kantor Pusat","34 Kanwil","Kancab terkait"],filters:[{key:"filter_program_id",nativeId:"NATIVE_FILTER-SAL-PRG-01",required:false,defaultValue:"Semua Program"},{key:"filter_date_id",nativeId:"NATIVE_FILTER-SAL-DATE-02",required:true,defaultValue:"MTD"},{key:"filter_kanwil_id",nativeId:"NATIVE_FILTER-SAL-KW-03",required:false,defaultValue:"Semua"}]},
  {id:"DB-004",name:"Distribusi & OTIF",domain:"Distribusi",type:"Operational",status:"Review",version:"v5",owner:"Divisi Supply Chain",updated:"28 Agu 2026 · 16:20",height:4860,widgets:13,placements:1,blivId:"53a2cc0e-86c0-48b5-9d88-b32d44c6f292",embedUrl:"https://bliv-dashboard.bulog.internal/embed/distribusi",auth:"Signed guest token",refresh:180,description:"Shipment, ETA, moda, koridor, OTIF, biaya angkut, dan exception distribusi.",roles:["Command Center","Divisi Supply Chain"],organizations:["Kantor Pusat","34 Kanwil"],filters:[{key:"filter_route_id",nativeId:"NATIVE_FILTER-DST-RTE-01",required:false,defaultValue:"Semua Koridor"},{key:"filter_mode_id",nativeId:"NATIVE_FILTER-DST-MOD-02",required:false,defaultValue:"Semua Moda"}]},
  {id:"DB-005",name:"Keuangan Supply Chain",domain:"Keuangan",type:"Executive",status:"Published",version:"v7",owner:"Divisi Keuangan",updated:"28 Agu 2026 · 15:08",height:4380,widgets:12,placements:2,blivId:"97066dbf-80db-45f5-9a1a-1ab3349d62d4",embedUrl:"https://bliv-dashboard.bulog.internal/embed/keuangan",auth:"Role proxy",refresh:900,description:"Pendapatan, biaya supply chain, piutang, pendanaan, dan budget versus actual.",roles:["Direksi","Divisi Keuangan","Auditor"],organizations:["Kantor Pusat"],filters:[{key:"filter_period_id",nativeId:"NATIVE_FILTER-FIN-PER-01",required:true,defaultValue:"YTD"},{key:"filter_cost_center_id",nativeId:"NATIVE_FILTER-FIN-CC-02",required:false,defaultValue:"Nasional"}]},
  {id:"DB-006",name:"Safety Stock & Aging",domain:"Persediaan",type:"Analytical",status:"Draft",version:"v3",owner:"Divisi Persediaan",updated:"27 Agu 2026 · 11:12",height:3920,widgets:10,placements:0,blivId:"85b30e5b-ec58-4b15-adb7-12fa09d00320",embedUrl:"https://bliv-dashboard.bulog.internal/embed/safety-aging",auth:"Signed guest token",refresh:600,description:"Coverage days, FEFO, aging lot, risiko turun mutu, dan rekomendasi redistribusi.",roles:["Command Center","Divisi Persediaan","Kanwil"],organizations:["Kantor Pusat","Pilot 12 Kanwil"],filters:[{key:"filter_commodity_id",nativeId:"NATIVE_FILTER-SSA-KOM-01",required:true,defaultValue:"Beras CBP"},{key:"filter_warehouse_id",nativeId:"NATIVE_FILTER-SSA-WH-02",required:false,defaultValue:"Semua Gudang"}]},
];

const blivWidgets=[
  ["WGT-001","Total Stok Nasional","KPI","National Persediaan","chart_1042","4×2","5 menit","Published"],
  ["WGT-002","Peta Okupansi Gudang","Map","National Persediaan","chart_1088","8×5","5 menit","Published"],
  ["WGT-003","Aging Lot CBP","Stacked Bar","Safety Stock & Aging","chart_1115","6×4","10 menit","Review"],
  ["WGT-004","Realisasi Pengadaan YTD","Combo Chart","National Pengadaan","chart_2021","6×4","10 menit","Published"],
  ["WGT-005","OTIF per Koridor","Heatmap","Distribusi & OTIF","chart_3077","6×4","3 menit","Review"],
  ["WGT-006","Piutang Aging","Table","Keuangan Supply Chain","chart_4028","8×4","15 menit","Published"],
];
const blivMenuMappings=[
  ["BERANDA / National Dashboard","National Persediaan","Main","Direksi, Command Center","01","Aktif"],
  ["Persediaan / Ringkasan Persediaan","National Persediaan","Tab: Persediaan","Persediaan, Kanwil","10","Aktif"],
  ["Persediaan / Safety Stock","Safety Stock & Aging","Tab: Safety Stock","Persediaan, Kanwil","14","Aktif"],
  ["Pengadaan / Ringkasan Pengadaan","National Pengadaan","Main","Pengadaan, Kanwil","20","Aktif"],
  ["Distribusi / Monitoring Pengiriman","Distribusi & OTIF","Tab: Shipment","Supply Chain, Kanwil","40","Review"],
  ["Keuangan / Ringkasan Keuangan","Keuangan Supply Chain","Main","Keuangan, Direksi","50","Aktif"],
];
const blivSavedViews=[
  ["VIEW-014","Risiko stok timur","National Persediaan","Papua, Maluku · Beras CBP · coverage <14 hari","Command Center","Tim","Setiap 06:00","28 Agu · 08:12"],
  ["VIEW-021","Serapan panen Jawa","National Pengadaan","Jatim, Jateng, Jabar · GKP/GKG · 30 hari","Divisi Pengadaan","Tim","Senin 07:00","28 Agu · 07:42"],
  ["VIEW-033","Exception OTIF laut","Distribusi & OTIF","Moda laut · OTIF <95% · open shipment","Control Room","Privat","Tidak","27 Agu · 18:10"],
  ["VIEW-046","Piutang >30 hari","Keuangan Supply Chain","B2B/RPK · aging >30 hari · outstanding","Divisi Keuangan","Direksi","Bulanan","26 Agu · 13:04"],
];

function DashboardManagementPage({mode,onSwitch,onNotify}:{mode:DashboardManagementMode;onSwitch:(mode:DashboardManagementMode)=>void;onNotify:(message:string)=>void}){
  const [query,setQuery]=useState("");const [status,setStatus]=useState("Semua Status");const [selectedId,setSelectedId]=useState(blivDashboards[0].id);const [editor,setEditor]=useState<"create"|"edit"|null>(null);const [filterCount,setFilterCount]=useState(4);const [layout,setLayout]=useState("Direksi Harian");const [selectedSlot,setSelectedSlot]=useState(1);
  const selected=blivDashboards.find(x=>x.id===selectedId)??blivDashboards[0];
  const tabs:[DashboardManagementMode,string,ComponentType<{size?:number}>][]=[["dashboards","Dashboard",BarChart3],["widgets","Widget",Boxes],["menus","Menu",ListFilter],["savedViews","Saved View",Eye],["executiveLayout","Executive Layout",Layers3]];
  const dashboardRows=blivDashboards.filter(x=>(status==="Semua Status"||x.status===status)&&`${x.name} ${x.domain} ${x.owner}`.toLowerCase().includes(query.toLowerCase()));
  const openCreate=(label:string)=>{if(mode==="dashboards")setEditor("create");else onNotify(`${label} baru siap dikonfigurasi`)};
  return <main className="dashboard-management-page"><header className="dm-header"><div><span>ADMINISTRATION / DASHBOARD MANAGEMENT</span><h1>{mode==="dashboards"?"BLIV Dashboard":mode==="widgets"?"Widget Registry":mode==="menus"?"Menu & Placement":mode==="savedViews"?"Saved View":"Executive Layout"}</h1><p>{mode==="dashboards"?"Kelola koneksi, embed, native filter, akses, versi, dan publikasi dashboard BLIV.":mode==="widgets"?"Daftarkan chart BLIV sebagai widget reusable dengan ukuran, refresh, dan hak akses terukur.":mode==="menus"?"Petakan dashboard dan tab BLIV ke navigasi SCCT berdasarkan role serta cakupan organisasi.":mode==="savedViews"?"Kelola filter tersimpan, default view, jadwal snapshot, dan pembagian tampilan lintas tim.":"Susun tampilan eksekutif dari widget tervalidasi dengan maker-checker dan jadwal publikasi."}</p></div><div className="dm-sync"><i/><span><small>BLIV gateway</small><strong>Connected · 182 ms</strong></span><em>Config sync 28 Agu 2026 · 21:14</em></div></header><nav className="dm-tabs">{tabs.map(([key,label,Icon])=><button key={key} className={mode===key?"active":""} onClick={()=>onSwitch(key)}><Icon size={16}/>{label}{key==="dashboards"&&<em>{blivDashboards.length}</em>}</button>)}</nav><section className="dm-kpis">{(mode==="dashboards"?[["Dashboard terdaftar","36","29 published"],["Embed sehat","34/36","2 perlu review"],["Native filter","128","97% mapped"],["Menu placement","74","5 draft"]]:mode==="widgets"?[["Widget aktif","142","12 domain"],["BLIV chart mapped","136","95,8%"],["Median render","1,4 dtk","target <2 dtk"],["Perlu review","7","owner notified"]]:mode==="menus"?[["Menu aktif","86","5 section"],["Placement dashboard","74","12 native page"],["Role coverage","18","RBAC synced"],["Konflik mapping","2","perlu tindakan"]]:mode==="savedViews"?[["Saved view","284","167 shared"],["Default organisasi","42","34 Kanwil"],["Snapshot terjadwal","31","99,2% sukses"],["Belum dipakai","18",">90 hari"]]:[["Layout aktif","7","3 executive"],["Slot terpakai","46/60","76,7%"],["Ready publish","2","menunggu approval"],["Avg load","1,8 dtk","target <2,5 dtk"]]).map(([label,value,note],i)=><article key={label} className={i===2?"good":""}><span>{label}</span><strong>{value}</strong><small>{note}</small></article>)}</section>
  {mode!=="executiveLayout"&&<section className="dm-toolbar"><label><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Cari ${mode==="dashboards"?"dashboard, domain, owner":"nama, ID, atau scope"}…`}/></label><select value={status} onChange={e=>setStatus(e.target.value)}><option>Semua Status</option><option>Published</option><option>Review</option><option>Draft</option></select><button onClick={()=>onNotify("Konfigurasi diekspor untuk audit")}><Download size={15}/>Ekspor</button><button className="primary" onClick={()=>openCreate(mode==="dashboards"?"Dashboard":mode==="widgets"?"Widget":mode==="menus"?"Placement":"Saved view")}><Plus size={16}/>Tambah {mode==="dashboards"?"Dashboard":mode==="widgets"?"Widget":mode==="menus"?"Placement":"Saved View"}</button></section>}
  {mode==="dashboards"&&<div className="dm-workspace"><section className="dm-table"><header><div><span>DASHBOARD CATALOG</span><h2>Embed BLIV terdaftar</h2></div><small>{dashboardRows.length} konfigurasi ditampilkan</small></header><div className="dm-table-head"><b>Dashboard / ID</b><b>Domain / Type</b><b>Widget</b><b>Placement</b><b>Owner / Update</b><b>Versi</b><b>Status</b><b/></div>{dashboardRows.map(x=><button key={x.id} className={selected.id===x.id?"selected":""} onClick={()=>setSelectedId(x.id)}><span><strong>{x.name}</strong><small>{x.id} · {x.blivId.slice(0,13)}…</small></span><span><strong>{x.domain}</strong><small>{x.type}</small></span><b>{x.widgets}</b><b>{x.placements}</b><span><strong>{x.owner}</strong><small>{x.updated}</small></span><em>{x.version}</em><i className={`status ${x.status.toLowerCase()}`}>{x.status}</i><ChevronRight size={16}/></button>)}</section><aside className="dm-detail"><header><div><span>EMBED 360°</span><h2>{selected.name}</h2><p>{selected.id} · {selected.version}</p></div><i className={selected.status.toLowerCase()}>{selected.status}</i></header><div className="dm-health"><span><i/><b>Embed sehat</b><small>HTTP 200 · {selected.refresh}s refresh</small></span><strong>182<small>ms</small></strong></div><p>{selected.description}</p><dl><div><dt>Dashboard ID</dt><dd>{selected.blivId}</dd></div><div><dt>Ukuran embed</dt><dd>100% × {selected.height}px</dd></div><div><dt>Authentication</dt><dd>{selected.auth}</dd></div><div><dt>Origin</dt><dd>BLIV internal · allow-listed</dd></div></dl><h3>Native filter mapping</h3><div className="dm-filter-list">{selected.filters.map(x=><span key={x.key}><b>{x.key}</b><small>{x.nativeId}</small><em>{x.required?"Required":"Optional"}</em></span>)}</div><h3>Akses</h3><div className="dm-chips">{selected.roles.map(x=><span key={x}>{x}</span>)}</div><div className="dm-detail-actions"><button onClick={()=>onNotify(`Preview aman ${selected.name} dibuka`)}><Eye size={15}/>Preview</button><button onClick={()=>setEditor("edit")}><Settings size={15}/>Edit konfigurasi</button></div></aside></div>}
  {mode==="widgets"&&<section className="dm-registry"><header><div><span>WIDGET REGISTRY</span><h2>Komponen BLIV reusable</h2></div><button onClick={()=>onNotify("Validasi batch widget dijalankan")}><FlaskConical size={15}/>Validasi batch</button></header><div className="dm-registry-head"><b>Widget / ID</b><b>Visual</b><b>Sumber Dashboard</b><b>Chart ID</b><b>Grid</b><b>Refresh</b><b>Status</b><b/></div>{blivWidgets.filter(x=>x.join(" ").toLowerCase().includes(query.toLowerCase())).map(x=><div className="dm-registry-row" key={x[0]}><span><strong>{x[1]}</strong><small>{x[0]}</small></span><span>{x[2]}</span><span>{x[3]}</span><code>{x[4]}</code><b>{x[5]}</b><span>{x[6]}</span><i className={`status ${x[7].toLowerCase()}`}>{x[7]}</i><button onClick={()=>onNotify(`${x[1]} dibuka pada widget builder`)}><Settings size={15}/></button></div>)}<footer><ShieldCheck size={16}/><span>Widget hanya dapat dipublikasikan bila chart ID valid, ukuran grid tersedia, query lulus batas waktu, dan role scope sudah ditetapkan.</span></footer></section>}
  {mode==="menus"&&<div className="dm-menu-layout"><section className="dm-registry"><header><div><span>NAVIGATION MAPPING</span><h2>Menu SCCT ↔ dashboard BLIV</h2></div><button onClick={()=>onNotify("Urutan menu disimpan sebagai draf")}><Save size={15}/>Simpan urutan</button></header><div className="dm-menu-head"><b>Path menu</b><b>Dashboard</b><b>Target</b><b>Role scope</b><b>Urutan</b><b>Status</b><b/></div>{blivMenuMappings.filter(x=>x.join(" ").toLowerCase().includes(query.toLowerCase())).map((x,i)=><div className="dm-menu-row" key={x[0]}><span><ListFilter size={15}/><strong>{x[0]}</strong></span><b>{x[1]}</b><span>{x[2]}</span><small>{x[3]}</small><code>{x[4]}</code><i className={x[5]==="Aktif"?"active":"review"}>{x[5]}</i><span className="actions"><button onClick={()=>onNotify(`${x[0]} dinaikkan`)}><ChevronUp size={14}/></button><button onClick={()=>onNotify(`${x[0]} diturunkan`)}><ChevronDown size={14}/></button><button onClick={()=>onNotify(`Mapping ${x[0]} dibuka`)}><Settings size={14}/></button></span></div>)}<footer><AlertTriangle size={16}/><span>Dua mapping masih menunjuk versi dashboard Review. Publikasi menu produksi akan ditahan hingga dashboard sumber disetujui.</span></footer></section><aside className="dm-policy"><span>PLACEMENT POLICY</span><h2>Resolver akses</h2>{[["1","Validasi menu aktif"],["2","Cocokkan permission role"],["3","Turunkan scope organisasi"],["4","Terbitkan signed embed token"],["5","Terapkan fallback page"]].map(x=><div key={x[0]}><i>{x[0]}</i><b>{x[1]}</b></div>)}<button onClick={()=>onNotify("Preview role Direksi dibuka")}><Eye size={15}/>Preview sebagai role</button></aside></div>}
  {mode==="savedViews"&&<section className="dm-registry"><header><div><span>PERSONALIZATION</span><h2>Saved view & snapshot</h2></div><button onClick={()=>onNotify("Retensi saved view dibuka")}><ShieldCheck size={15}/>Retention policy</button></header><div className="dm-view-head"><b>View / ID</b><b>Dashboard</b><b>Filter tersimpan</b><b>Owner</b><b>Visibilitas</b><b>Jadwal</b><b>Terakhir dibuka</b><b/></div>{blivSavedViews.filter(x=>x.join(" ").toLowerCase().includes(query.toLowerCase())).map(x=><div className="dm-view-row" key={x[0]}><span><strong>{x[1]}</strong><small>{x[0]}</small></span><b>{x[2]}</b><small>{x[3]}</small><span>{x[4]}</span><i>{x[5]}</i><span>{x[6]}</span><span>{x[7]}</span><button onClick={()=>onNotify(`${x[1]} diduplikasi`)}><Share2 size={14}/></button></div>)}<footer><Sparkles size={16}/><span>Saved view organisasi dapat menjadi default hanya setelah pemilik data menyetujui filter, periode, dan cakupan aksesnya.</span></footer></section>}
  {mode==="executiveLayout"&&<><section className="dm-layout-toolbar"><div><span>LAYOUT TEMPLATE</span><select value={layout} onChange={e=>setLayout(e.target.value)}><option>Direksi Harian</option><option>Command Center 24/7</option><option>Kanwil Executive</option></select><em>v6 · Draft perubahan</em></div><span><button onClick={()=>onNotify("Preview resolusi 1920×1080 dibuka")}><Eye size={15}/>Preview</button><button onClick={()=>onNotify("Layout disimpan sebagai draf")}><Save size={15}/>Simpan Draf</button><button className="primary" onClick={()=>onNotify("Layout diajukan kepada approver Direksi")}><Send size={15}/>Ajukan Publikasi</button></span></section><div className="dm-layout-workspace"><section className="dm-canvas"><header><div><span>{layout}</span><small>12-column grid · 1920×1080 · auto refresh 5 menit</small></div><em>EDIT MODE</em></header><div className="dm-grid">{[[1,"Supply Chain Health Score","KPI nasional","wide"],[2,"Prioritas Penanganan","Alert Center",""],[3,"Persediaan Nasional","National Persediaan","wide"],[4,"Realisasi Pengadaan","National Pengadaan",""],[5,"OTIF & Distribusi","Distribusi & OTIF",""],[6,"Dampak Keuangan","Keuangan Supply Chain","wide"]].map(([id,title,source,size])=><button key={id} className={`${size} ${selectedSlot===id?"selected":""}`} onClick={()=>setSelectedSlot(Number(id))}><small>SLOT {id}</small><BarChart3 size={20}/><strong>{title}</strong><span>{source}</span><em>{id===2?"Alert overlay ON":"Auto refresh"}</em></button>)}</div></section><aside className="dm-slot-editor"><span>SLOT CONFIGURATION</span><h2>Slot {selectedSlot}</h2><label><small>Widget source</small><select defaultValue={selectedSlot===2?"Prioritas Penanganan":"Total Stok Nasional"}><option>Total Stok Nasional</option><option>Prioritas Penanganan</option><option>Realisasi Pengadaan YTD</option><option>OTIF per Koridor</option><option>Piutang Aging</option></select></label><label><small>Lebar grid</small><select defaultValue="6 kolom"><option>3 kolom</option><option>4 kolom</option><option>6 kolom</option><option>12 kolom</option></select></label><label><small>Click-through</small><select><option>Detail dashboard</option><option>Saved view</option><option>Alert Center</option><option>Dinonaktifkan</option></select></label><div className="dm-toggle-row"><span><b>Auto refresh</b><small>Ikuti policy widget</small></span><input type="checkbox" defaultChecked/></div><div className="dm-toggle-row"><span><b>Alert overlay</b><small>Critical & high only</small></span><input type="checkbox" defaultChecked={selectedSlot===2}/></div><button onClick={()=>onNotify(`Konfigurasi slot ${selectedSlot} diterapkan`)}>Terapkan ke slot</button><footer><Clock3 size={15}/><span>Terakhir diubah Aly Sangadji · 28 Agu 2026 20:42 WIB</span></footer></aside></div></>}
  <footer className="dm-disclaimer"><ShieldCheck size={16}/><span><b>Kontrol embed produksi:</b> origin allow-list, CSP frame-src, short-lived signed token, row-level security, audit akses, versioning, dan maker-checker wajib aktif. Data halaman ini adalah konfigurasi demonstrasi yang merepresentasikan kebutuhan SCCT BULOG.</span></footer>
  {editor&&<div className="dm-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)setEditor(null)}}><section className="dm-modal" role="dialog" aria-modal="true" aria-labelledby="dm-editor-title"><header><div><span>{editor==="create"?"CREATE":"UPDATE"} BLIV DASHBOARD</span><h2 id="dm-editor-title">{editor==="create"?"Dashboard baru":selected.name}</h2><p>Konfigurasi sumber embed, native filter, akses, performa, dan workflow publikasi.</p></div><button onClick={()=>setEditor(null)} aria-label="Tutup editor"><X size={20}/></button></header><div className="dm-modal-body"><section><h3>1. Informasi dashboard</h3><div className="dm-form-grid"><label><span>Nama dashboard *</span><input defaultValue={editor==="edit"?selected.name:""} placeholder="Contoh: Persediaan Regional"/></label><label><span>Dashboard ID BLIV *</span><input defaultValue={editor==="edit"?selected.blivId:""} placeholder="UUID dashboard"/></label><label className="full"><span>Deskripsi</span><textarea defaultValue={editor==="edit"?selected.description:""} placeholder="Tujuan, pengguna, dan keputusan yang didukung"/></label><label><span>Domain</span><select defaultValue={selected.domain}><option>Persediaan</option><option>Pengadaan</option><option>Penjualan</option><option>Distribusi</option><option>Keuangan</option><option>Eksekutif</option></select></label><label><span>Tipe</span><select defaultValue={selected.type}><option>Main Dashboard</option><option>Operational</option><option>Analytical</option><option>Executive</option></select></label><label><span>Tinggi embed (px)</span><input type="number" defaultValue={editor==="edit"?selected.height:4200}/></label><label><span>Refresh interval</span><select defaultValue={String(selected.refresh)}><option value="180">3 menit</option><option value="300">5 menit</option><option value="600">10 menit</option><option value="900">15 menit</option></select></label></div></section><section><h3>2. Embed & keamanan</h3><div className="dm-form-grid"><label className="full"><span>Embed URL *</span><input defaultValue={editor==="edit"?selected.embedUrl:"https://bliv-dashboard.bulog.internal/embed/"}/></label><label><span>Authentication</span><select defaultValue={selected.auth}><option>Signed guest token</option><option>Role proxy</option><option>SSO pass-through</option></select></label><label><span>Token TTL</span><select defaultValue="300"><option value="300">5 menit</option><option value="600">10 menit</option><option value="900">15 menit</option></select></label></div><div className="dm-security-checks"><label><input type="checkbox" defaultChecked/><span><b>Origin allow-list</b><small>Hanya BLIV internal dan domain SCCT.</small></span></label><label><input type="checkbox" defaultChecked/><span><b>Row-level security</b><small>Region/Kanwil/Kancab dari session role.</small></span></label><label><input type="checkbox" defaultChecked/><span><b>Sandbox iframe</b><small>Scripts, forms, dan same-origin terkontrol.</small></span></label></div></section><section><header className="dm-section-head"><div><h3>3. Native filter mapping</h3><p>Nama parameter SCCT dipetakan ke native filter ID BLIV.</p></div><button onClick={()=>setFilterCount(x=>x+1)}><Plus size={14}/>Tambah mapping</button></header><div className="dm-filter-editor-head"><b>SCCT key</b><b>Native filter ID</b><b>Default</b><b>Required</b><b/></div>{Array.from({length:filterCount},(_,i)=>{const f=selected.filters[i];return <div className="dm-filter-editor-row" key={i}><input defaultValue={f?.key??`filter_custom_${i+1}_id`}/><input defaultValue={f?.nativeId??"NATIVE_FILTER-"}/><input defaultValue={f?.defaultValue??"Semua"}/><input type="checkbox" defaultChecked={f?.required}/><button onClick={()=>setFilterCount(x=>Math.max(1,x-1))}><X size={14}/></button></div>})}</section><section><h3>4. Akses & publikasi</h3><div className="dm-form-grid"><label><span>Role yang diizinkan</span><input defaultValue={selected.roles.join(", ")}/></label><label><span>Cakupan organisasi</span><input defaultValue={selected.organizations.join(", ")}/></label><label><span>Owner</span><input defaultValue={selected.owner}/></label><label><span>Approver</span><select><option>Head of Command Center</option><option>Direktur Supply Chain</option><option>Data Owner Domain</option></select></label></div><div className="dm-test-panel"><span><i/><b>Connection test siap</b><small>Validasi endpoint, response, filter ID, token, CSP, dan render time sebelum disimpan.</small></span><button onClick={()=>onNotify("Connection test berhasil · HTTP 200 · 182 ms")}><FlaskConical size={15}/>Test connection</button></div></section></div><footer><span><AlertTriangle size={15}/><small>Perubahan disimpan sebagai versi baru dan tidak langsung mengganti embed produksi.</small></span><button onClick={()=>setEditor(null)}>Batal</button><button onClick={()=>onNotify("Konfigurasi dashboard disimpan sebagai draf")}><Save size={15}/>Simpan Draf</button><button className="primary" onClick={()=>{setEditor(null);onNotify("Konfigurasi dashboard diajukan untuk approval")}}><Send size={15}/>Ajukan Publikasi</button></footer></section></div>}
  </main>;
}
...
export default function HomePage() {
  ...
  const [forecastReportOpen, setForecastReportOpen] = useState<ForecastReportMode | null>(null);
  ...
  const [dashboardManagementOpen, setDashboardManagementOpen] = useState<DashboardManagementMode | null>(null);
  ...
  function selectSidebarItem(label: string, parentLabel?: string) {
    ...
    const forecastModes:Record<string,ForecastReportMode>={"Demand & Supply Forecast":"demandSupply","Inventory Forecast":"inventory","Forecast Accuracy":"accuracy"};
    if(parentLabel==="Forecast Report"&&forecastModes[label]){
      setForecastReportOpen(forecastModes[label]);
      showToast(`${label} aktif`);
      return;
    }
    ...
    const dashboardManagementModes:Record<string,DashboardManagementMode>={"Dashboard":"dashboards","Widget":"widgets","Menu":"menus","Saved View":"savedViews","Executive Layout":"executiveLayout"};
    if(parentLabel==="Dashboard Management"&&dashboardManagementModes[label]){
      setDashboardManagementOpen(dashboardManagementModes[label]);
      showToast(`${label} aktif`);
      return;
    }
  ...
  return (
    <main className="app-shell">
      ...
      <section className={`workspace${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
        ...
        {forecastReportOpen&&<div className={sidebarCollapsed?"forecast-report-host sidebar-collapsed":"forecast-report-host"}><ForecastReportPage key={forecastReportOpen} mode={forecastReportOpen} onSwitch={(next)=>{setForecastReportOpen(next);const labels:Record<ForecastReportMode,string>={demandSupply:"Demand & Supply Forecast",inventory:"Inventory Forecast",accuracy:"Forecast Accuracy"};setActiveNav(labels[next]);setActiveNavParent("Forecast Report")}} onNotify={showToast}/></div>}
