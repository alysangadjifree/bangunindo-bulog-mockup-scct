...
type CaptchaState = { question: string; challenge: string; signature: string };

function LoginPage({ onAuthenticated }: { onAuthenticated: () => void }) {
  const [username,setUsername]=useState(""); const [password,setPassword]=useState(""); const [showPassword,setShowPassword]=useState(false); const [captcha,setCaptcha]=useState<CaptchaState|null>(null); const [captchaAnswer,setCaptchaAnswer]=useState(""); const [error,setError]=useState(""); const [submitting,setSubmitting]=useState(false); const [attempts,setAttempts]=useState(0);
  ...
  async function refreshCaptcha(){setCaptchaAnswer("");try{const response=await fetch("/api/demo-auth?action=captcha",{cache:"no-store"});if(!response.ok)throw new Error();setCaptcha(await response.json())}catch{setError("CAPTCHA tidak dapat dimuat. Silakan coba kembali.")}}
  useEffect(()=>{void refreshCaptcha()},[]);
  async function submit(event:React.FormEvent){event.preventDefault();if(!captcha||!passwordValid||!captchaAnswer){setError("Lengkapi semua input dan persyaratan keamanan.");return}if(attempts>=5){setError("Terlalu banyak percobaan. Muat ulang halaman untuk mencoba kembali.");return}setSubmitting(true);setError("");try{const response=await fetch("/api/demo-auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"login",username,password,captchaAnswer,challenge:captcha.challenge,signature:captcha.signature})});const data=await response.json();if(!response.ok){setAttempts((count)=>count+1);setError(data.error??"Login gagal.");await refreshCaptcha();return}onAuthenticated()}catch{setError("Koneksi ke layanan autentikasi gagal. Silakan coba kembali.")}finally{setSubmitting(false)}}
  return <main className="login-page"><section className="login-panel"><div className="login-brand"><span className="brand-mark"><i/><i/><i/></span><strong>bulog</strong></div><div className="login-heading"><span>SUPPLY CHAIN CONTROL TOWER</span><h1>Masuk ke SCCT BULOG</h1><p>Gunakan akun yang telah terdaftar untuk mengakses dashboard nasional.</p></div><form onSubmit={submit} noValidate><label><span>Username</span><div><UserRound size={17}/><input autoComplete="username" value={username} onChange={(e)=>setUsername(e.target.value)} placeholder="Masukkan username" required/></div></label><label><span>Password</span><div><ShieldCheck size={17}/><input type={showPassword?"text":"password"} autoComplete="current-password" value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="Masukkan password" required/><button type="button" onClick={()=>setShowPassword(!showPassword)} aria-label={showPassword?"Sembunyikan password":"Tampilkan password"}>{showPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></label>{password&&<div className="login-password-checks"><span className={passwordChecks.length?"ok":""}><Check size={11}/>Minimal 8 karakter</span><span className={passwordChecks.lower?"ok":""}><Check size={11}/>Huruf kecil</span><span className={passwordChecks.number?"ok":""}><Check size={11}/>Angka</span><span className={passwordChecks.special?"ok":""}><Check size={11}/>Simbol khusus</span></div>}<div className="login-captcha"><div><ShieldCheck size={18}/><span><b>Verifikasi keamanan</b><small>Selesaikan CAPTCHA berikut</small></span></div><div><strong>{captcha?.question??"Memuat…"} =</strong><input inputMode="numeric" value={captchaAnswer} onChange={(e)=>setCaptchaAnswer(e.target.value.replace(/\D/g,""))} aria-label="Jawaban CAPTCHA"/><button type="button" onClick={()=>void refreshCaptcha()} aria-label="Muat ulang CAPTCHA"><RotateCw size={16}/></button></div></div>{error&&<div className="login-error" role="alert"><AlertTriangle size={15}/>{error}</div>}<button className="login-submit" type="submit" disabled={submitting||!captcha}>{submitting?<><RotateCw size={16}/>Memverifikasi…</>:<><ShieldCheck size={16}/>Masuk Aman</>}</button><div className="login-help"><button type="button">Lupa password?</button><span>·</span><button type="button">Hubungi administrator</button></div></form><footer><span><ShieldCheck size={13}/>Koneksi terenkripsi</span><span>Demo SCCT · Akses terbatas</span></footer></section><aside className="login-visual"><div className="login-grid-art"><i/><i/><i/><i/><i/><i/></div><div className="login-visual-content"><span>PERUM BULOG · COMMAND CENTER</span><h2>Satu kendali untuk ketahanan pangan Indonesia</h2><p>Pantau persediaan, pengadaan, penyaluran, distribusi, keuangan, serta exception nasional secara terpadu.</p><div><article><strong>34</strong><span>Kanwil terpantau</span></article><article><strong>24/7</strong><span>Operational monitoring</span></article><article><strong>1</strong><span>National control tower</span></article></div></div><footer>SCCT BULOG · Sistem Demo Terproteksi</footer></aside></main>;
}
